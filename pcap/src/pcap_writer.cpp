...
PcapWriter::PcapWriter(const std::string& filename) : filename_(filename) {    
...
bool PcapWriter::write_packet(const uint8_t* data, uint32_t len, uint32_t ts_sec, uint32_t ts_usec) {
    if (!ok_) return false;
